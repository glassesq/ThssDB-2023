...
public class SelectPlan extends LogicalPlan {
  ...
  public QueryResult getResult(ArrayList<Table.TableMetadata> tables) {
    if (!useJoin) {
      if (!useWhere) {
        return getCondition(tables.get(0));
      }else {
        if (true/*TODO*/) { //单列主键，且WHERE子句查询主键时
          if (cmp_where.EQ() != null) return getEqual(tables.get(0));
          else if (cmp_where.LE() != null || cmp_where.LT() != null) return getLess(tables.get(0));
          else if (cmp_where.GE() != null || cmp_where.GT() != null) return getGreater(tables.get(0));
          else if (cmp_where.NE() != null) return getCondition(tables.get(0));
          else return null;
  ...
  public SelectPlan(ArrayList<SQLParser.ColumnFullNameContext> columns, ArrayList<String> tableNames) {
    super(LogicalPlanType.SELECT);
    this.columns = columns;
    this.tableNames = tableNames;
  }
  public SelectPlan(ArrayList<SQLParser.ColumnFullNameContext> columns, ArrayList<String> tableNames, SQLParser.ConditionContext condition_where, boolean useJoin) {
    super(LogicalPlanType.SELECT);
    this.columns = columns;
    this.tableNames = tableNames;
  ...
  public SelectPlan(ArrayList<SQLParser.ColumnFullNameContext> columns, ArrayList<String> tableNames, SQLParser.ConditionContext condition_on) {
    super(LogicalPlanType.SELECT);
    this.columns = columns;
    this.tableNames = tableNames;
  ...
  public SelectPlan(ArrayList<SQLParser.ColumnFullNameContext> columns, ArrayList<String> tableNames, SQLParser.ConditionContext condition_on, SQLParser.ConditionContext condition_where) {
    super(LogicalPlanType.SELECT);
    this.columns = columns;
    this.tableNames = tableNames;
    init_where(condition_where);
